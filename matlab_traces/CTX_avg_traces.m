% Average traces in a single animal
...
for k = 1:size(stimulus,1)
    start_index = find(time > stimulus{k,2}, 1 );
...
output_folder = fullfile(analyzed_root,'Traces');
...
xlabel('time (s)')
ylabel('\Delta F/F_0')
saveas(fig1,fullfile(output_folder,strcat(animal,'_',data_avg(n).neuron,'_ind.png')))
...
xlabel('time (s)')
ylabel('\Delta F/F_0')
saveas(fig2,fullfile(output_folder,strcat(animal,'_',data_avg(n).neuron,'_avg.png')))
close all
end
...
save(fullfile(analyzed_root,'avg_data.mat'),'data','data_avg','stimulus');
