function CTX_id_uint8_volume(dataset,raw_data_root,output_folder)
...
% Find a good range to make uint8
    ...
    file.data = zeros(S, 'uint8');
        ...
        slice = convert_to_uint8(green_vol);
        if size(size(slice),2) == 3 % standard case
        file.data(:,:,:) = slice;
        elseif size(size(slice),2) == 4 % case for accidental 2-frame landmark
        file.data(:,:,:) = slice(:,:,:,1);
        end
